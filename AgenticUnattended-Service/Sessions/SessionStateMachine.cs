...
public sealed class SessionStateMachine
{
    ...
    private readonly ILogger<SessionStateMachine> _logger;
    ...
    public SessionStateMachine(
        ...
        BeaconConfig config,
        TimeProvider time,
        ILogger<SessionStateMachine> logger
    )
    {
        _registry = registry;
        _monitor = monitor;
    ...
    public void OnWindowFocusChanged(nint hwnd, string processName)
    {
    ...
    )
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                await Task.Delay(TimeSpan.FromSeconds(_config.AfkThresholdSeconds), _time, cts.Token);
            }
    ...
    private void Publish(BeaconEvent evt)
    {
        _bus.Publish(evt);
    }
